...
bool Utils::hasIntersection(const Ray& cameraRay, const std::vector<Shape*>& objectsList) //or Shadow Ray
{
    ...
    for(size_t objIdx = 0; objIdx < objectsList.size(); objIdx++)
    {
        const Shape *obj = objectsList.at(objIdx);
...
bool Utils::getClosestIntersection(const Ray& cameraRay, const std::vector<Shape*>& objectsList, Intersection& its) //or Closest Hit Ray
{
    ...
    closest.itsPoint = Vector3D(std::numeric_limits<double>::infinity());
    closest.normal = Vector3D((0,0,0));
    closest.shape = nullptr;

    for(size_t objIdx = 0; objIdx < objectsList.size(); objIdx++){
        const Shape *obj = objectsList.at(objIdx);
        Intersection IS;
        if(obj->rayIntersect(cameraRay, IS)){
            double dist = (IS.itsPoint - cameraRay.o).length();
            if(dist < (closest.itsPoint - cameraRay.o).length()){
                closest = IS;
            }
        }
    }
